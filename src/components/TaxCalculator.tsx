...
export default function TaxCalculator() {
  ...
  const calculateTax = () => {
    ...
    const rentRelief = rentAmount
      ? Math.min(500000, Number(rentAmount) * 0.2)
      : 0;
    ...
    let taxableIncome = Math.max(
      0,
      annualIncome - rentRelief - pensionDeduction - nhfDeduction
    );
    let taxPayable = 0;
    let breakdown = [];
  ...
  const calculateOldTax = (annualIncome: number, pension: number) => {
    ...
    const consolidatedRelief =
      Math.max(200000, annualIncome * 0.01) + annualIncome * 0.2;
  ...
  const oldTax =
    result && employmentType === "salaried"
      ? calculateOldTax(result.annualIncome, result.pensionDeduction)
      : null;
  const monthlyNewTakeHome = result
    ? (result.annualIncome -
        result.taxPayable -
      ...
      12
    : 0;
  const monthlyOldTakeHome =
    result && oldTax !== null
      ? (result.annualIncome - oldTax - result.pensionDeduction) / 12
      : 0;
                ...
                <div className="grid grid-cols-3 gap-3">
                  {["salaried", "freelancer", "business"].map((type) => (
                    <button
                      ...
                      className={`px-4 py-3 text-sm font-medium rounded-2xl border transition-all ${
                        employmentType === type
              ...
              {employmentType === "salaried" ? (
                    ...
                    <h3 className="text-sm font-bold text-gray-900 mb-4 flex items-center uppercase tracking-wider">
                      ...
                      <div className="group relative ml-2">
                        ...
                        <div className="hidden group-hover:block absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 p-3 bg-gray-900 text-white text-xs rounded-xl shadow-xl z-10">
                          These help lower the tax you pay. Leave as 0 if they
                          don't apply to you.
                        </div>
                      </div>
                    </h3>
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-gray-500 text-sm max-w-xs">
                    The {employmentType} calculator is under development. Check
                    back soon!
                  </p>
                </div>
              )}
            ...
            {!result ? (
              <div className="flex-grow flex flex-col items-center justify-center text-center py-12 relative z-10">
                ...
                <p className="text-gray-500 text-sm max-w-[200px]">
                  Fill in your details on the left to see your simple tax
                  breakdown.
                </p>
              </div>
            ) : (
              <div className="space-y-8 relative z-10 animate-fade-in-up">
                ...
                {employmentType === "salaried" && oldTax !== null && (
                  <div className="bg-white rounded-2xl p-5 shadow-lg shadow-primary/5 border border-primary/10">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <p className="text-lg font-bold text-gray-900">
                          ₦
                          {monthlyOldTakeHome.toLocaleString(undefined, {
                      ...
                      <div>
                        ...
                        <p className="text-lg font-bold text-primary">
                          ₦
                          {monthlyNewTakeHome.toLocaleString(undefined, {
                            maximumFractionDigits: 0,
                          })}
                        </p>
                      </div>
                    ...
                    <div
                      className={`p-3 rounded-xl text-sm font-medium flex items-center justify-center ${
                        monthlyGain >= 0
                    ...
                    >
                      {monthlyGain >= 0 ? (
                        <>
                          🎉 You keep ₦
                          {monthlyGain.toLocaleString(undefined, {
                            maximumFractionDigits: 0,
                          })}{" "}
                          more / month
                        </>
                        ...
                        <>
                          ⚠️ You pay ₦
                          {Math.abs(monthlyGain).toLocaleString(undefined, {
                            maximumFractionDigits: 0,
                          })}{" "}
                          more / month
                        </>
                ...
                <div className="text-center">
                  ...
                  <p className="text-5xl font-bold text-primary tracking-tight">
                    ₦
                    {result.taxPayable.toLocaleString(undefined, {
                      maximumFractionDigits: 0,
                    })}
                  </p>
                ...
                <div className="space-y-4 bg-white rounded-2xl p-5 shadow-sm border border-primary/5">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs text-gray-400 pl-2">
                    ...
                    <div className="text-right">
                      -₦{result.pensionDeduction.toLocaleString()}
                    </div>
                    <div className="text-right">
                      -₦{result.pensionDeduction.toLocaleString()}
                    </div>
                    {deductNhf && (
                      <>
                        <div>NHF ({nhfRate}%)</div>
                        <div className="text-right">
                          -₦{result.nhfDeduction.toLocaleString()}
                        </div>
                      </>
                    )}
                    {result.rentRelief > 0 && (
                      <>
                        <div>Rent Relief</div>
                        <div className="text-right">
                          -₦{result.rentRelief.toLocaleString()}
                        </div>
                      </>
                    )}
                      ...
                      {result.breakdown.map((item, idx) =>
                        item.amount > 0 || item.rate === "0%" ? (
                          ...
                          >
                            ...
                            <div className="flex items-center">
                              <span className="bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded text-[10px] mr-2 font-medium">
                                {item.rate}
                              </span>
                              <span className="font-medium text-gray-900">
